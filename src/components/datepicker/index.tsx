...
export default function Datepick({ selectDate, initialDate }: DatepickProps) {
  const DAYS = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
  const [dayCount, setDayCount] = useState<Array<number>>([]);
  const [blankDays, setBlankDays] = useState<Array<number>>([]);
  const [showDatepicker, setShowDatepicker] = useState(true);
  const [datepickerHeaderDate, setDatepickerHeaderDate] = useState(initialDate);
  ...
  const isToday = (date: number) => isEqual(
  ...
  const setDateValue = (date: number) => () => {
    ...
    selectDate(`${datepickerHeaderDate.getFullYear()}-${datepickerHeaderDate.getMonth() + 1}-${date}`)
  ...
  const isSelectedMonth = (month: number) => isEqual(
  ...
  return (
    ...
    <div className="relative">
      {showDatepicker && (
      <div
        className="bg-white  rounded-lg shadow p-4 absolute top-0 left-0"
        style={{ width: '17rem' }}
      >
